...
if (!empty($token)) {
    ...
    $tokenQuery = firstRaw("SELECT id, email FROM user_admin WHERE forgotToken='$token'");
    ...
    if (!empty($tokenQuery)) {
        ...
        if (isPost()) {
            ...
            if (empty($errors)) {
                ...
                $updateStatus = update('user_admin', $dateUpdate, "id=$userId");
                if ($updateStatus) {
            ...
            redirect('?module=admin&action=auth/reset&token=' . $token);
        } //End isPost()
